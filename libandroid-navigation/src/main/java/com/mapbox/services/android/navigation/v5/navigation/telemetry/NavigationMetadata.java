...
public class NavigationMetadata implements Parcelable {
  ...
  public NavigationMetadata(Date startTimestamp, int distanceCompleted, int distanceRemaining, int durationRemaining,
    ...
    this.batteryLevel = TelemetryUtils.obtainBatteryLevel(MapboxTelemetry.applicationContext);
    this.screenBrightness = NavigationUtils.obtainScreenBrightness();
    this.batteryPluggedIn = TelemetryUtils.isPluggedIn(MapboxTelemetry.applicationContext);
    this.connectivity = TelemetryUtils.obtainCellularNetworkType(MapboxTelemetry.applicationContext);
    this.audioType = NavigationUtils.obtainAudioType();
    this.applicationState = TelemetryUtils.obtainApplicationState(MapboxTelemetry.applicationContext);
  }
